    ...
    <div className="min-h-screen flex text-slate-900">
      ...
      <aside className="w-64 bg-white border-r p-6 hidden md:block">
        ...
        <nav className="space-y-2 font-medium">
          <div className="flex items-center gap-3 p-3 bg-blue-50 text-blue-600 rounded-lg"><Layout size={20}/> Dashboard</div>
      ...
      <main className="flex-1 p-8">
        <header className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-slate-500">Connecting talent to USA-based opportunities.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          ...
          <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-xl shadow-slate-200">
            ...
            <p className="text-sm font-bold opacity-60 uppercase tracking-widest text-slate-400">Satisfaction</p>
          </div>
        </div>
        ...
        <div className="bg-white border rounded-[2rem] p-8 shadow-sm">
          ...
          <div className="w-full">
            {talent.map((p) => (
              <div key={p.id} className="flex items-center justify-between py-4 border-b last:border-0 hover:bg-slate-50 px-4 rounded-xl transition-all cursor-pointer">
                <div>
                  <p className="font-bold">{p.name}</p>
                  <p className="text-sm text-slate-500">{p.role}</p>
                </div>
                <div className="flex gap-4 items-center">
                  <span className="bg-blue-50 text-blue-600 px-3 py-1 rounded-full text-xs font-bold tracking-widest uppercase">{p.stack}</span>
                  <span className="text-xs font-bold text-green-500 uppercase tracking-widest">{p.status}</span>
                </div>
              </div>
            ))}
          </div>
